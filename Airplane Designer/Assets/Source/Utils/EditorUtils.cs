...
namespace Utils
{
    public static class EditorUtils
    {
        ...
        public static List<GameObject> GetAllChildren(GameObject parent, bool includeParent)
        {
            List<GameObject> retVal = GetAllChildren(parent);
            if (!includeParent)
            {
                retVal.Remove(parent);
            }
            return retVal;
        }
        ...
        /// Returns only the GameObjects directly parented to the supplied "parent".
        public static List<GameObject> GetDirectChildren(GameObject parent)
        {
            List<GameObject> results = new List<GameObject>();

            for (int i = 0, count = parent.transform.childCount; i < count; i++)
            {
                results.Add(parent.transform.GetChild(i).gameObject);
            }

            return results;
        }

        public static Vector3 GetGameObjectGroupCenter(List<GameObject> gameObjects)
        {
            Vector3 centerPos = Vector3.zero;
            int numObjects = gameObjects.Count;
            for (int i = 0; i < numObjects; i++)
            {
                centerPos += gameObjects[i].transform.position;
            }

            centerPos.x /= numObjects;
            centerPos.y /= numObjects;
            centerPos.z /= numObjects;
        ...
        public static Vector3 GetGroundPositionFromCamRay(Ray camRay)
        {
            ...
            MathEditorUtils.LinePlaneIntersection(out spawnPos, camRay.origin, camRay.direction,
        ...
        private static int layerMaskObjects = -1;
        public static Vector3 GetObjectCollisionFromScreenPoint(Vector3 screenPoint)
        {
            Vector3 returnPos = Vector3.zero;
            RaycastHit hitDataGizmo;

            if (layerMaskObjects == -1)
            {
                string[] objectLayer = new string[] { EditorConstants.OBJECT_LAYER, };
        ...
        public static void SetObjectColor(Transform parent, Color color)
        {
            SetObjectColor(parent, color, true);
        }

        public static void SetObjectColor(Transform parent, Color color, bool affectChildren)
        {
            if (parent != null)
            {
